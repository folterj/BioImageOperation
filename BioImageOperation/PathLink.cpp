...
void PathLink::draw(Mat* image, Scalar color, int max, bool animate, double scaling, double scale) {
	int x1 = (int)(this->x1 * scaling);
	int y1 = (int)(this->y1 * scaling);
	int x2 = (int)(this->x2 * scaling);
	int y2 = (int)(this->y2 * scaling);
	Point point;
	double animPos2;
	...
	if (animate) {
		...
		drawMarker(*image, point, color, MarkerTypes::MARKER_CROSS, (int)(2 * scale), (int)scale, LineTypes::LINE_AA);
	} else {
		line(*image, Point(x1, y1), Point(x2, y2), color, (int)scale, LineTypes::LINE_AA);
	}
}
